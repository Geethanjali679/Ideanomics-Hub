...
const EditIdeaPage = () => {
  ...
  useEffect(() => {
    ...
    }).catch(() => navigate('/creator/ideas')).finally(() => setLoading(false));
  }, [id, navigate]);
  ...
  return (
    <DashboardPage>
      <Box sx={{ maxWidth: 760, mx: 'auto' }}>
        ...
        <Typography color="text.secondary" sx={{ mb: 3 }}>Update your idea details</Typography>
        <Card>
          <CardContent sx={{ p: 3.5 }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <TextField fullWidth multiline rows={3} label="Short Description" name="shortDescription" value={form.shortDescription} onChange={handleChange} required sx={{ mb: 2.5 }} helperText={`${form.shortDescription.length}/300`} inputProps={{ maxLength: 300 }} />
              <TextField fullWidth multiline rows={7} label="Full Description" name="fullDescription" value={form.fullDescription} onChange={handleChange} required sx={{ mb: 2.5 }} />
              <TextField fullWidth label="Tags (comma-separated)" name="tags" value={form.tags} onChange={handleChange} sx={{ mb: 3.5 }} />
              <Box sx={{ display: 'flex', gap: 2 }}>
                <Button variant="outlined" onClick={() => navigate('/creator/ideas')} sx={{ flex: 1 }}>Cancel</Button>
                <Button type="submit" variant="contained" startIcon={<Save />} disabled={saving} sx={{ flex: 2 }}>{saving ? 'Saving...' : 'Save Changes'}</Button>
              </Box>
            </form>
          </CardContent>
        </Card>
